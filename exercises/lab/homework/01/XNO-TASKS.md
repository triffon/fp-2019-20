...
### `(play curr-board curr-sign)`
`play` взима дъска `curr-board` и знак `curr-sign`, който трябва да сложим сега и връща индекс на следващия ход, като наредена двойка.

Моят замисъл за имплементацията на тази функция е да се използва `minmax` "алгоритъма". По-долу ще опиша какво представлява той неформално
и какви стъпки/под-функции бих/съм написал аз за да го реализирам. Нямам нищо против вие да ползвате друга идея (алгоритъм), стига тя да изпълнява
следните две условия:

1. Да се справя поне толкова добре колкото `minmax` решението.
2. Да е лесно скалируемо, ако задачата беше за дъска `nxn`, а не за `3x3` например. (Това очевидно изключва решения с 1234567890 вложени `if`-а.)
