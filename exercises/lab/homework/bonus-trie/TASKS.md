...
В него може да сложите изход от `Show`, генерирана по подразбиране, за да получите "разкрасена" такава.
...
`"асдф"` като ключ, да не се пази отделен връх за всеки от символите `'a'`, `'s'`, `'d'`, `'f'`), с цел простота.
