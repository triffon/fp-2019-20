...
Възможно е да срещнете случай(/и) в рекурсията си, който не би трябвало да е възможен,
...
и да разкараме `error`-а (при конкретната имплементация към която се отнася).
...
Освен това ни дава повече гаранции за програмата ни - не е нужно вече да "помним" (т.е. забравим
след 1 седмица), че сме направили проверка за празен списък - можем да си го гарантираме използвайки типове,
проверени от компилатора.

Например можем да напишем вече `head`, без да се притесняваме че ще гръмне:
```haskell
head :: NonEmpty a -> a
head (x :| _) = x
```
